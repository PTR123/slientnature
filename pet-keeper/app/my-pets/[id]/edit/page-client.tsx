...
export default function EditPetPage({ petId }: { petId: string }) {
  ...
  const loadPet = async () => {
    try {
      const pet = await apiClient.getPetById(petId);
      setFormData({
        name: pet.name,
        species: pet.species,
        birthDate: pet.birthDate.split('T')[0],
        acquisitionDate: pet.acquisitionDate.split('T')[0],
        image: pet.image || ''
      });
      if (pet.image) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.updatePet(petId, formData);
  ...
  return (
    <div className="min-h-screen bg-cream-50 py-8">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-2xl">
        <Link href={`/my-pets/${petId}`} className="inline-flex items-center text-forest-600 hover:text-forest-700 mb-6">
